    ...
    try:
        ...
        def truncate(x, n=160):
            if isinstance(x, str) and len(x) > n:
                return x[: n - 1] + "…"
            return x  # Only truncate strings > 160 char
...
def save_output(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        msg = (
            f"Saved items to {csv_path} "
            f"({'appended' if csv_exists else 'created'}) with {len(df)} new rows."
        )
        print(msg)
    except Exception as e:
    ...
    if json_path.exists():
        try:
            with json_path.open("r", encoding="utf-8") as f:  # r = read mode
                ...
                if isinstance(existing, list):  # checking if existing is a list
